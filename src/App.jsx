...
export default function App() {
  // --- CONSTANTS / CONFIG ---
  // Since Admin is removed, these defaults are hardcoded or read once.
  const appName = localStorage.getItem('cbt_appName') || "DANIEL'S ANATOMY CBT APP";
  const testTitle = localStorage.getItem('cbt_testTitle') || "ANAT 213: GENERAL EMBRYO AND GENETICS";
  const testDuration = parseInt(localStorage.getItem('cbt_duration') || "20", 10);
  const marksPerQuestion = parseInt(localStorage.getItem('cbt_marks') || "2", 10);

  // --- STATE ---
  const [view, setView] = useState('welcome'); // 'welcome', 'test', 'result'
  ...
  const [answers, setAnswers] = useState(() => {
    // Attempt to load saved answers for resume functionality
    const saved = localStorage.getItem('cbt_saved_answers');
    return saved ? JSON.parse(saved) : {};
  });
  ...
  // --- EFFECT: PERSIST ANSWERS & GUEST ID ---
  useEffect(() => { localStorage.setItem('cbt_saved_answers', JSON.stringify(answers)); }, [answers]);
  ...
  // --- SMART RESUME & TIMER LOGIC ---
  useEffect(() => {
    const savedEndTime = localStorage.getItem('cbt_endTime');

    // 1. If we are just loading (view is welcome), check if we should resume or submit
    if (view === 'welcome' && savedEndTime) {
        const endTime = parseInt(savedEndTime, 10);
        const now = Date.now();
        
        if (now >= endTime) {
            // Time expired while away -> Force Submit
            handleSubmit(); 
        } else {
            // Time remains -> Resume Test
  ...
  useEffect(() => {
    if (view === 'test') {
        // Push state initially so the back button has something to pop
        window.history.pushState(null, '', window.location.href);

        const handlePopState = (event) => {
            // User pressed back
            const confirmQuit = window.confirm('Are you sure you want to quit the test? Your progress will be lost.');
            
            if (confirmQuit) {
                // Quit: Clean up and go to welcome
                localStorage.removeItem('cbt_endTime');
                localStorage.removeItem('cbt_saved_answers');
                setAnswers({});
                setView('welcome');
            } else {
                // Stay: Push state again to re-arm the back button trap
                window.history.pushState(null, '', window.location.href);
            }
        };
  ...
  const startTest = () => {
    if (questions.length === 0) {
      alert("No questions loaded. Please check Questions.csv");
      return;
    }
    ...
    // Set End Time for Smart Resume
    const endTime = Date.now() + (durationSec * 1000);
    localStorage.setItem('cbt_endTime', endTime.toString());
    
    // Shuffle Questions newly for a fresh start
    const randomizedQuestions = shuffleArray([...questions]);
    setQuestions(randomizedQuestions);
    // Update stored questions to match the current shuffled order for consistency on resume
  ...
  if (view === 'test') {
    ...
    if (!question) return <div>Loading...</div>; // Safety check
