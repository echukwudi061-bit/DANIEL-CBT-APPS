...
export default function App() {
  // --- 1. CONFIGURATION (Static Defaults) ---
  // Since Admin is removed, these are now static defaults or simple persistent configs.
  const appName = "DANIEL'S ANATOMY CBT APP";
  const testTitle = "ANAT 213: GENERAL EMBRYO AND GENETICS";
  const testDuration = 20; // Minutes
  const marksPerQuestion = 2;

  // --- 2. LAZY STATE INITIALIZATION (Smart Resume Logic) ---
  
  // A. Check for active session to determine initial View
  const [view, setView] = useState(() => {
    const savedEndTime = localStorage.getItem('cbt_endTime');
    // If endTime exists and is in the future, go straight to test
  ...
  // --- 3. EFFECTS: DATA & PERSISTENCE ---

  // Load Questions.csv on Mount if empty
  useEffect(() => {
    if (questions.length === 0) {
        fetch('/Questions.csv')
            .then(response => {
                if(response.ok) return response.text();
                throw new Error("Questions.csv not found");
            })
            .then(text => {
                const loadedQuestions = parseCSVString(text);
            ...
            .catch(err => console.log("Auto-load info:", err.message));
    }
  }, []);

  // Persist Answers and Index immediately when they change
  useEffect(() => { 
    if (view === 'test') {
      localStorage.setItem('cbt_answers', JSON.stringify(answers));
      localStorage.setItem('cbt_qIndex', currentQIndex.toString());
    }
  }, [answers, currentQIndex, view]);

  // --- 4. ROBUST BACK BUTTON PROTECTION ---
  useEffect(() => {
    const handlePopState = (event) => {
      if (view === 'test') {
        // The user pressed Back. Browser has already popped the state.
        // We interrupt this flow.
        const confirmQuit = window.confirm("Are you sure you want to quit the test? Progress will be lost.");
        
        if (confirmQuit) {
          handleExitSession(); // Clear storage and go to welcome
        } else {
          // User wants to stay. We must RE-PUSH the state to restore the "forward" history
          // and trap the back button again.
    ...
    if (view === 'test') {
      // Add a history entry when we enter 'test' view so there is something to pop
      window.history.pushState(null, '', window.location.href);
  ...
  useEffect(() => {
    ...
    if (view === 'test' && timeLeft > 0) {
      timer = setInterval(() => {
        // We recalculate based on end time to prevent drift, 
        // but for simple countdowns, decrementing is often smoother visually
        // providing we check the absolute end time occasionally. 
        // Here we just decrement for smoothness since we set initial state accurately.
        setTimeLeft(prev => {
          if (prev <= 1) {
            handleSubmit();
            return 0;
  ...
  const startTest = () => {
    if (questions.length === 0) {
      alert("No questions available! Please ensure Questions.csv is in the public folder.");
      return;
    }
    ...
    localStorage.setItem('cbt_questions', JSON.stringify(randomizedQuestions));
    ...
    localStorage.setItem('cbt_qIndex', '0');
  ...
  const handleSubmit = () => {
    ...
    // Clear Session so refresh doesn't put them back in test
    localStorage.removeItem('cbt_endTime');
    localStorage.removeItem('cbt_answers');
    localStorage.removeItem('cbt_qIndex');
      ...
      <div className="min-h-screen bg-gray-50 flex items-center justify-center p-4 font-sans text-gray-900">
        <div className="bg-white p-8 rounded-2xl shadow-xl max-w-md w-full text-center border border-blue-100 relative">
          ...
          <div className="text-left bg-gray-50 p-4 rounded-lg text-sm text-gray-700 mb-8 space-y-2">
            ...
            <p className="flex items-center"><FileText className="w-4 h-4 mr-2"/> <strong>Questions:</strong> {questions.length > 0 ? questions.length : 'Loading...'}</p>
          ...
          <button 
            ...
            className="w-full bg-blue-600 hover:bg-blue-700 disabled:bg-gray-400 text-white font-bold py-4 rounded-xl shadow-lg transition transform active:scale-95 flex items-center justify-center mb-6"
          >
                        ...
                        <div className="flex justify-between mt-8 md:mt-10 pt-6 border-t border-gray-100">
                            <button 
                                ...
                                onClick={() => setCurrentQIndex(prev => prev - 1)}
                                 classNaame="flex items-center text-gray-500 hover:text-blue-600 disabled:opacity-30 disabled:hover:text-gray-500 font-medium text-sm md:text-base"
                            >
