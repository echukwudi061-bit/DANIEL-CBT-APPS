...
const Toast = ({ message, show }) => (
  <div className={`fixed bottom-10 left-1/2 transform -translate-x-1/2 bg-gray-900 text-white px-6 py-3 rounded-full shadow-2xl z-50 transition-opacity duration-300 font-bold text-sm ${show ? 'opacity-100' : 'opacity-0 pointer-events-none'}`}>
    {message}
  </div>
);

export default function App() {
  // --- 1. CONFIGURATION STATE (Immutable via UI now) ---
  // We still load these to prevent the app from breaking, but there is no UI to change them.
  ...
  // --- 3. VIEW STATE (LAZY INIT FOR NO FLASH) ---
  const [view, setView] = useState(() => {
    // A. Check if a test is currently running (survives refresh)
    const savedEndTime = localStorage.getItem('cbt_endTime');
    if (savedEndTime && parseInt(savedEndTime, 10) > Date.now()) {
      return 'test';
    }
    // B. Check if a result is waiting in session (survives refresh, deleted on close)
  ...
  // Load answers from localStorage to survive refresh
  const [answers, setAnswers] = useState(() => {
    const saved = localStorage.getItem('cbt_answers');
    return saved ? JSON.parse(saved) : {};
  });

  // Derived state for timer (calculated, not decremented state)
  const [timeLeft, setTimeLeft] = useState(0);
  
  const [isMobileMenuOpen, setIsMobileMenuOpen] = useState(false);
  const [showToast, setShowToast] = useState(false); // For "Double tap to exit"
  ...
  // --- LOGIC: ADVANCED BACK BUTTON HANDLING ---
  useEffect(() => {
    const handlePopState = (event) => {
      // SCENARIO A: TEST VIEW (Confirmation Trap)
      if (view === 'test') {
        // The browser has already popped the state, so we are "back".
        // We show the confirm dialog.
        const confirmLeave = window.confirm("Are you sure you want to quit? Your progress will be lost.");
        
        if (confirmLeave) {
          handleExitToWelcome(); // Clean up and go home
        } else {
          // USER STAYED: Crucial Fix - Re-push state immediately to restore the "trap"
      ...
      // SCENARIO B: RESULT VIEW (Double Tap)
        ...
        if (timeDiff < 2000) {
          ...
          // First press: Show Toast and Re-push state
    ...
    // Only add listener if we are in a protected view
    if (view === 'test' || view === 'result') {
      // Push the initial state so we have something to "pop"
      window.history.pushState(null, '', window.location.href);
  ...
  const startTest = () => {
    if (questions.length === 0) {
      alert("No questions available! Please ensure Questions.csv is uploaded.");
      return;
    }
  ...
  const handleAnswerSelect = (qId, option) => {
    ...
    // Persist immediately to survive crash
  ...
  const handleSubmit = () => {
    ...
    const resultData = {
      ...
      questions: questions, // Pass specific shuffled order to results
      answers: answers
    };

    // 1. Save to SESSION Storage (Survives Refresh, dies on Close)
    sessionStorage.setItem('cbt_currentResult', JSON.stringify(resultData));

    // 2. Clear Local Storage (Test is over)
                ...
                <div className="max-w-3xl mx-auto w-full">
                    <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-4 md:p-10 min-h-[50vh]">
                        <div className="mb-4 md:mb-6 flex justify-between items-center">
                            ...
                            {/* BADGE 2: Mobile Counter (Synced Style) */}
  ...
  // ---------------- RENDER: RESULT ----------------
  if (view === 'result') {
    // Retreive data from Session Storage
