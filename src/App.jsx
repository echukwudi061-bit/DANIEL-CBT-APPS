...
export default function App() {
  ...
  // REQ 1: PERSIST INDEX (Fixes refresh reset)
  ...
  // REQ 1 & 2: LAZY TIMER INIT (Fixes red flash on refresh)
        ...
        fetch('/Questions.csv')
            ...
            .then(text => {
                ...
                lines.forEach((line, idx) => {
                   // Existing Regex logic for CSV parsing
  ...
  // --- REQ 3: ROBUST BACK BUTTON HANDLING ---
  useEffect(() => {
    const handlePopState = (event) => {
      ...
      if (view === 'test') {
        ...
        if (confirmLeave) {
          ...
          // CRUCIAL FIX: Wait 50ms before re-pushing. 
          // Browsers sometimes ignore immediate pushState during blocking events.
          setTimeout(() => {
      ...
      // SCENARIO B: RESULT VIEW (Double Tap)
        ...
        if (timeDiff < 2000) {
          ...
          // Trap logic with small delay for stability
          setTimeout(() => {
             window.history.pushState(null, '', window.location.href);
          }, 50);
        }
    ...
    if (view === 'test' || view === 'result') {
      // Push initial state to have something to pop
      window.history.pushState(null, '', window.location.href);
  ...
  const handleSubmit = () => {
    ...
    // 2. Clear Local Storage Test Data (Test is over)
    localStorage.removeItem('cbt_endTime');
    localStorage.removeItem('cbt_answers');
    localStorage.removeItem('cbt_currentIndex'); // Clear stored index
                ...
                <div className="max-w-3xl mx-auto w-full">
                    <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-4 md:p-10 min-h-[50vh]">
                        <div className="mb-4 md:mb-6 flex justify-between items-center">
                            ...
                            {/* BADGE 2: Mobile Counter (Synced Style per Requirements) */}
