...
export default function App() {
  ...
  // 1. Questions - Init empty or from localStorage
  ...
  const parseCSVString = (text) => {
    ...
    lines.forEach((line, idx) => {
      ...
      if (parts.length >= 6 && idx > 0) {
         ...
         if (clean(parts[0])) { // Only add if question text exists
            newQuestions.push({
  ...
  useEffect(() => {
    // Only fetch if we don't have questions stored yet
  ...
  const startTest = () => {
    if (questions.length === 0) {
      alert("No questions available! Please login as admin and add some or upload a CSV.");
      return;
    }
  ...
  const handleCSVUpload = (e) => {
    ...
    reader.onload = (event) => {
      ...
      const newQuestions = parseCSVString(text);
  ...
  const handleResetDefaults = () => {
    if(window.confirm("Reset all settings and questions? This clears local changes and attempts to reload Questions.csv.")) {
        localStorage.clear();
                   ...
                   {testResults.length === 0 ? (
                     <tr>
                       ...
                       <tr key={res.id} className="hover:bg-gray-50">
                         <td className="p-3 font-bold text-blue-800">{res.guestId || "N/A"}</td>
